...
dat |>  group_by(end_date) |>
  ...
  ggplot(aes(x=end_date, y=rate)) + geom_line(data=dat, aes(x=end_date, y=cases/population*10^4, group=state, color=region, alpha=0.5)) + geom_line(aes(x=end_date, y=rate))  + geom_vline(aes(xintercept=as.Date("2020-12-15")), linetype=2)
...
tests2 <- tests |> mutate(date=mdy(date), tests=parse_number(tests))
