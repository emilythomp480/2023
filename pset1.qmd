...
```{r}
plot(x=avg$year, y=avg$rate, xlab="Year", ylab="Measles Cases per 10,000 people per year", main="Measles Case Rate in the U.S.", )
abline(v=1963, col="blue")
legend("topright",legend="Introduction of Measles Vaccine", col="blue", lty="solid")
...
us_contagious_diseases2 <- us_contagious_diseases |>
  mutate(avg=count*52/weeks_reporting/population*10^4) |>
  filter(weeks_reporting>0) |>
  filter(disease=="Measles" & !state %in% c("Hawaii","Alaska")) |>
  arrange(year, state)
...
abline(v=1963, col="blue")
legend("topright",legend="Introduction of Measles Vaccine", col="blue", lty="solid")
lines(x=us_contagious_diseases2$year, y=us_contagious_diseases2$rate, type="l")
...
mmwr_to_date <- function(year, week){
    jan_4 <- make_date(year, 1, 4)  
  
  round_date(jan_4, unit = "week") + weeks(week-1)
